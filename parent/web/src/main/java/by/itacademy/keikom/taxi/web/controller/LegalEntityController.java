...
public class LegalEntityController {
	...
	@Autowired
	private ILegalEntityServices legalEntityServise;
	...
	public ModelAndView viewList(final HttpServletRequest req,
		...
		final List<LegalEntity> currentPageList = legalEntityServise.getAll(legalEntityFilter);
		listModel.setList(currentPageList.stream().map(toDTOConverter).collect(Collectors.toList()));
		listModel.setTotalCount(legalEntityServise.getCount(legalEntityFilter));
	...
	public String save(@Validated @ModelAttribute("legalEntityForm") final LegalEntityDTO legalEntityForm,
		...
		if (result.hasErrors()) {
			...
			legalEntityServise.save(fromDTOConverter.apply(legalEntityForm));
	...
	public ModelAndView viewDetails(@PathVariable(name = "id", required = true) final Integer id) {
		...
		hashMap.put("legalEntityForm", toDTOConverter.apply(legalEntityServise.get(id)));
	...
	public ModelAndView edit(@PathVariable(name = "id", required = true) final Integer id) {
		...
		return new ModelAndView("legalEntity.edit", "legalEntityForm",
				toDTOConverter.apply(legalEntityServise.get(id)));
	}
}
